...
use crate::models::{
    CodexProviderSettings, ModeInfo, ModelInfo, PromptContent, ProviderInfo, ProviderType, StreamChunk,
};
...
impl CodexAdapter {
    ...
    pub fn with_settings(settings: &CodexProviderSettings) -> Self {
        Self {
            child: None,
            cmd_tx: None,
            acp_session_id: None,
    ...
    fn spawn_process(
        ...
        let npx_path = ProviderDetector::find_in_path("npx").ok_or_else(|| {
            AppError::Provider(
                "npx not found in PATH. Please install Node.js to use Codex ACP.".to_string(),
            )
        })?;
        ...
        println!("[CodexAdapter] Executing command:");
        println!("  Command: {} @zed-industries/codex-acp", npx_path.display());
        println!("  Working directory: {}", worktree_path.display());
        println!("  Environment variables:");
        for (key, value) in &env {
            if key.starts_with("OPENAI_") || key.starts_with("CODEX_") || key == "PATH" {
                println!("    {}={}", key, value);
            }
        }
        ...
        let mut child = tokio::process::Command::new(npx_path.as_os_str())
            ...
            .map_err(|e| {
                AppError::Provider(format!(
                    "Failed to spawn npx @zed-industries/codex-acp: {}",
                    e
                ))
            })?;
...
impl ProviderAdapter for CodexAdapter {
    ...
    async fn start_session(
        ...
        println!(
            "[CodexAdapter] Starting ACP session for {}",
            session_id
        );
        ...
        println!(
            "[CodexAdapter] ACP session established: {}",
        ...
        println!(
            "[CodexAdapter] Handshake complete: available_models = {:?}, current_model = {:?}, available_modes = {:?}, current_mode = {:?}",
    ...
    async fn resume_session(
        ...
        println!(
            "[CodexAdapter] Resuming ACP session {} for {} (worktree: {}, project: {})",
            acp_session_id,
            session_id,
        ...
        println!(
            "[CodexAdapter] ACP session resumed: {}",
            handshake.session_id
        );
    ...
    async fn send_message(&mut self, content: Vec<PromptContent>) -> AppResult<()> {
        ...
        if content.len() == 1 {
            if let PromptContent::Text { text } = &content[0] {
                ...
                match reply_rx.await {
                    Ok(Ok(())) => {
                        println!(
                            "[CodexAdapter] Sent permission response: option_id={}",
                            text.trim()
                        );
                        return Ok(());
                    }
        ...
        tokio::spawn(async move {
            match reply_rx.await {
                Ok(Ok(())) => {}
                Ok(Err(e)) => {
                    eprintln!("[CodexAdapter] Prompt error: {}", e);
                }
                Err(_) => {
                    eprintln!("[CodexAdapter] Prompt reply channel closed");
                }
            }
        });
    ...
    async fn cancel(&mut self) -> AppResult<()> {
        println!("[CodexAdapter] cancel() called");
        let cmd_tx = self
            .cmd_tx
            .as_ref()
            .ok_or_else(|| AppError::Provider("Session not started".to_string()))?;
        ...
        println!("[CodexAdapter] Sending Cancel command for session: {}", acp_session_id);
        let (reply_tx, reply_rx) = tokio::sync::oneshot::channel();
        ...
        println!("[CodexAdapter] Cancel command result: {:?}", result);
        result
    }

    async fn terminate(&mut self) -> AppResult<()> {
        println!("[CodexAdapter] Terminating session");
