...
use crate::models::{
    ModeInfo, ModelInfo, PromptContent, ProviderInfo, ProviderType, QwenCodeProviderSettings, StreamChunk,
};
...
impl QwenCodeAdapter {
    ...
    pub fn with_settings(settings: &QwenCodeProviderSettings) -> Self {
        Self {
            child: None,
            cmd_tx: None,
            acp_session_id: None,
    ...
    /// Spawn the qwen ACP process. Returns (child, stdin, stdout, stderr).
    fn spawn_process(
        &self,
        worktree_path: &Path,
    ) -> AppResult<(
        ...
        let mut child = tokio::process::Command::new(&cli_path)
            ...
            .envs(&env)
            .spawn()
            .map_err(|e| AppError::Provider(format!("Failed to spawn qwen --acp: {}", e)))?;
...
#[async_trait]
impl ProviderAdapter for QwenCodeAdapter {
    fn provider_type(&self) -> ProviderType {
        ProviderType::QwenCode
    }

    fn detect(&self) -> AppResult<ProviderInfo> {
        Ok(ProviderDetector::detect_provider(&ProviderType::QwenCode, None))
    }
    ...
    async fn start_session(
        ...
        println!("[QwenCodeAdapter] Starting ACP session for {}", session_id);
        ...
        println!(
            "[QwenCodeAdapter] ACP session established: {}",
        ...
        println!(
            "[QwenCodeAdapter] Handshake complete: available_models = {:?}, current_model = {:?}",
    ...
    async fn resume_session(
        ...
        println!(
            "[QwenCodeAdapter] Resuming ACP session {} for {} (worktree: {}, project: {})",
            acp_session_id,
            session_id,
        ...
        println!(
            "[QwenCodeAdapter] ACP session resumed: {}",
            handshake.session_id
        );
    ...
    async fn send_message(&mut self, content: Vec<PromptContent>) -> AppResult<()> {
        ...
        if content.len() == 1 {
            if let PromptContent::Text { text } = &content[0] {
                ...
                match reply_rx.await {
                    Ok(Ok(())) => {
                        println!(
                            "[QwenCodeAdapter] Sent permission response: option_id={}",
                            text.trim()
                        );
                        return Ok(());
                    }
        ...
        tokio::spawn(async move {
            match reply_rx.await {
                Ok(Ok(())) => {}
                Ok(Err(e)) => {
                    eprintln!("[QwenCodeAdapter] Prompt error: {}", e);
                }
                Err(_) => {
                    eprintln!("[QwenCodeAdapter] Prompt reply channel closed");
                }
            }
        });
    ...
    async fn terminate(&mut self) -> AppResult<()> {
        println!("[QwenCodeAdapter] Terminating session");
