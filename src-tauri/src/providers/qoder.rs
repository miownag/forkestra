...
use crate::models::{
    ModeInfo, ModelInfo, PromptContent, ProviderInfo, ProviderType, QoderProviderSettings, StreamChunk,
};
...
impl QoderAdapter {
    ...
    pub fn with_settings(settings: &QoderProviderSettings) -> Self {
        Self {
            child: None,
            cmd_tx: None,
            acp_session_id: None,
    ...
    /// Spawn the qodercli ACP process. Returns (child, stdin, stdout, stderr).
    fn spawn_process(
        &self,
        worktree_path: &Path,
    ) -> AppResult<(
        ...
        let mut child = tokio::process::Command::new(&cli_path)
            ...
            .envs(&env)
            .spawn()
            .map_err(|e| AppError::Provider(format!("Failed to spawn qodercli --acp: {}", e)))?;
...
impl ProviderAdapter for QoderAdapter {
    ...
    fn detect(&self) -> AppResult<ProviderInfo> {
        Ok(ProviderDetector::detect_provider(&ProviderType::Qoder, None))
    }
    ...
    async fn start_session(
        ...
        println!("[QoderAdapter] Starting ACP session for {}", session_id);
        ...
        println!(
            "[QoderAdapter] ACP session established: {}",
        ...
        println!(
            "[QoderAdapter] Handshake complete: available_models = {:?}, current_model = {:?}",
    ...
    async fn resume_session(
        ...
        println!(
            "[QoderAdapter] Resuming ACP session {} for {} (worktree: {}, project: {})",
            acp_session_id,
            session_id,
        ...
        println!(
            "[QoderAdapter] ACP session resumed: {}",
            handshake.session_id
        );
    ...
    async fn send_message(&mut self, content: Vec<PromptContent>) -> AppResult<()> {
        ...
        if content.len() == 1 {
            if let PromptContent::Text { text } = &content[0] {
                ...
                match reply_rx.await {
                    Ok(Ok(())) => {
                        println!(
                            "[QoderAdapter] Sent permission response: option_id={}",
                            text.trim()
                        );
                        return Ok(());
                    }
        ...
        tokio::spawn(async move {
            match reply_rx.await {
                Ok(Ok(())) => {}
                Ok(Err(e)) => {
                    eprintln!("[QoderAdapter] Prompt error: {}", e);
                }
                Err(_) => {
                    eprintln!("[QoderAdapter] Prompt reply channel closed");
                }
            }
        });
    ...
    async fn terminate(&mut self) -> AppResult<()> {
        println!("[QoderAdapter] Terminating session");
