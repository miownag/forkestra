...
use crate::models::{
    ModeInfo, ModelInfo, OpenCodeProviderSettings, PromptContent, ProviderInfo, ProviderType, StreamChunk,
};
...
impl OpenCodeAdapter {
    ...
    pub fn with_settings(settings: &OpenCodeProviderSettings) -> Self {
        Self {
            child: None,
            cmd_tx: None,
            acp_session_id: None,
    ...
    /// Spawn the opencode ACP process. Returns (child, stdin, stdout, stderr).
    fn spawn_process(
        &self,
        worktree_path: &Path,
    ) -> AppResult<(
        ...
        let mut child = tokio::process::Command::new(&cli_path)
            ...
            .envs(&env)
            .spawn()
            .map_err(|e| AppError::Provider(format!("Failed to spawn opencode acp: {}", e)))?;
...
impl ProviderAdapter for OpenCodeAdapter {
    ...
    fn detect(&self) -> AppResult<ProviderInfo> {
        Ok(ProviderDetector::detect_provider(&ProviderType::OpenCode, None))
    }
    ...
    async fn start_session(
        ...
        println!("[OpenCodeAdapter] Starting ACP session for {}", session_id);
        ...
        println!(
            "[OpenCodeAdapter] ACP session established: {}",
        ...
        println!(
            "[OpenCodeAdapter] Handshake complete: available_models = {:?}, current_model = {:?}",
    ...
    async fn resume_session(
        ...
        println!(
            "[OpenCodeAdapter] Resuming ACP session {} for {} (worktree: {}, project: {})",
            acp_session_id,
            session_id,
        ...
        println!(
            "[OpenCodeAdapter] ACP session resumed: {}",
            handshake.session_id
        );
    ...
    async fn send_message(&mut self, content: Vec<PromptContent>) -> AppResult<()> {
        ...
        if content.len() == 1 {
            if let PromptContent::Text { text } = &content[0] {
                ...
                match reply_rx.await {
                    Ok(Ok(())) => {
                        println!(
                            "[OpenCodeAdapter] Sent permission response: option_id={}",
                            text.trim()
                        );
                        return Ok(());
                    }
        ...
        tokio::spawn(async move {
            match reply_rx.await {
                Ok(Ok(())) => {}
                Ok(Err(e)) => {
                    eprintln!("[OpenCodeAdapter] Prompt error: {}", e);
                }
                Err(_) => {
                    eprintln!("[OpenCodeAdapter] Prompt reply channel closed");
                }
            }
        });
    ...
    async fn terminate(&mut self) -> AppResult<()> {
        println!("[OpenCodeAdapter] Terminating session");
